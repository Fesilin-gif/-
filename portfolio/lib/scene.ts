 ...
 */
export const PHASE = {
  /** Отъезд камеры: transform .rig и локальное положение панорамы
      внутри его обёртки идут по всему этому окну — от огромного
      стартового кадра (и широкой, полноэкранной панорамы) до
      финального адаптивного размера (и панорамы, подогнанной точно
      под проём). */
  pullBack: [0, 0.8],
  /** Появление арки с девушкой: на входе (progress 0) её не видно —
      только пейзаж, — непрозрачность фигуры арки идёт по этому окну.
      Должно закончиться раньше, чем .rig (в рамках PHASE.pullBack)
      сожмётся настолько, что его проём перестанет перекрывать экран
      с запасом, — иначе на пути отъезда мелькнёт край маски. */
  figureIn: [0, 0.22],
 ...
 * готического проёма (cover, focus — точка интереса вида).
 */
 ...
 * to — localViewRect(view); идёт по той же фазе, что и .rig
 * (PHASE.pullBack), поэтому оба движения — одно на другое — заведомо
 * синхронны.
 */
