 ...
 * Готовность и запуск
 * --------------------
 * Арка появляется на экране сразу, как только страница стала
 * интерактивной, — ещё ДО готовности картинок и до конца ожидания:
 * посетитель не смотрит на голый белый фон, а сразу видит саму сцену
 * (арку, девушку, пейзаж в проёме) в её стартовом положении. Под аркой
 * — надпись «Загрузка…» с тонкой индикаторной полоской (без выдуманных
 * процентов, полоска просто бежит). Экран «Загрузка…» держится не
 * меньше INTRO.minLoadingMs (считая с открытия страницы, а не с
 * готовности картинок) — на быстрой сети или с картинками из кэша
 * браузера готовность может наступить почти сразу, и без этого порога
 * посетитель не успевает понять, что вообще происходит, прежде чем
 * экран сменится. Дальше, без клика и без прокрутки, сам начинается
 * влёт — из того же самого положения арки, в котором она стояла всё
 * время ожидания, без скачка.
 *
 * Ту самую пару кадров между первой отрисовкой (сервер отдал разметку,
 * но JS ещё не выполнился) и моментом, когда эффект успел посчитать
 * раскладку и применить transform, закрывает .intro__curtain — иначе
 * мелькнула бы нерасположенная (натурального размера, у левого верхнего
 * угла) арка. Curtain пропадает, как только раскладка применена И маска
 * проёма готова (см. проверку готовности маски ниже) — то есть почти
 * сразу, а не в конце всего ожидания.
 *
...
export default function Intro() {
  ...
  /** Функция apply(t), посчитанная под текущий экран — хранится, чтобы
   *  её можно было переиспользовать и для стартового кадра (t=0), и
   *  для пересчёта при resize/повороте экрана во время ожидания. */
  const applyRef = useRef<((rawT: number) => void) | null>(null);
  ...
  /** Раскладка на t=0 применена хотя бы раз — арка стоит на своём
   *  месте, можно её проявлять. */
  const [positioned, setPositioned] = useState(false);
  const allReady = ready.figure && ready.landscape && ready.mask;
  /* Арку проявляем, только когда она и расположена, и маска проёма уже
     готова (см. заголовок компонента) — иначе одним кадром мелькнёт
     непрозрачный проём. */
  const archVisible = positioned && ready.mask;
  ...
  /* Пока идёт ожидание, арка уже видна на экране — если размер окна
     меняется (поворот телефона, ресайз), пересчитываем тот же
     стартовый кадр заново, чтобы она не сдвинулась рывком в момент
     начала влёта. */
  useEffect(() => {
    if (phaseState !== 'loading' || !motionAllowedRef.current) return;
    const handleResize = () => {
      const apply = prepareFlight();
      if (apply) {
        applyRef.current = apply;
  ...
  return (
    <div
      className={`intro${archVisible ? ' intro--arch-visible' : ''}`}
      ref={introRef}
